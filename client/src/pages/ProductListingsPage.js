    ...
    const heads = [
        ...
        fetch(`http://localhost:3000/products/${productId}`, {
          ...
          .then((result) => {
            if (result.success) {
              ...
              serializeProductData(updatedProducts);
            } else {
      ...
      useEffect(() => {
        fetch("http://localhost:3000/products")
          ...
          .then((data) => {
            setProductList(data);
            serializeProductData(data);
          })
          .catch((error) => console.error("Error fetching products:", error));
      }, []);
    
      const serializeProductData = (products) => {
            ...
            actions: [
              ...
              },
              {
                ...
                buttonStyle: {
                  ...
                  hoverColor: "var(--secondary-red-hover)",
                },
                callback: () => deleteProduct(product._id),
              },
            ],
          });
        });
      ...
      const sortData = (dropDownValue, key) => {
        if (dropDownValue === "a-z" || dropDownValue === "low-high") {
          setserializedProductList(
            sortBy(originalSerializedProductList, key, true)
          );
        } else {
          setserializedProductList(
            sortBy(originalSerializedProductList, key, false)
          );
        }
      };
  ...
  return (
    <div className="product_listings page">
      ...
      <SearchBox
        ...
        onChange={(e) =>
          setserializedProductList(
            findEntries(originalSerializedProductList, e.target.value)
          )
        }
      />
      <hr />
      <div className="product_listings-dropdowns">
        <span>Sort By: </span>
        <DropDown1
          name="alphabetical_dropdown"
          options={sortOptionsName}
          onChange={(e) => {
            sortData(e.target.value, "name");
          }}
        />
        <DropDown1
          name="price_dropdown"
          options={sortOptionsPrice}
          onChange={(e) => {
        ...
        <Popup
          child={
            <ViewProductScreen
              ...
              onCloseClick={() => setisPopupVisibile(false)}
            />
          }
          onOutsideClick={() => setisPopupVisibile(false)}
        />
      ) : null}
    </div>
  );
